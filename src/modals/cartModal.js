...
import template from '../components/cart/cartTemplate.hbs';
import { removeFromCart } from '../components/cart/cart';
import { getTotal } from '../components/cart/cart';
...
export const cartModal = () => {
  ...
  const instance = basicLightbox.create(
    ...
    <span class ="allProducts"><b>All products in cart: </b>${cart.totalQuantity}</span>
  ...
  const removeDish = (e) => {
    ...
    e.target.classList.contains('remove')) {
      ...
      getTotal();
      document.querySelector('.summary').innerHTML = `<b>Summary: </b>${cart.totalSum}`;
      document.querySelector('.allProducts').innerHTML = `<b>All products in cart: </b>${cart.totalQuantity}`;
    };
    console.log(cart);
  };
